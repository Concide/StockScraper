...
const parse = html => {
  ...
  return {
    pe: Number(pe["P/E"]),
    peForward: Number(forwardPE["Forward P/E"]),
    epsTTM: Number(epsTTM["EPS (ttm)"]),
    epsNextQuarter: Number(epsNextQuarter["EPS next Q"]),
    ...
    bookPerShare: Number(bookPerShare["Book/sh"]),
    cashPerShare: Number(cashPerShare["Cash/sh"]),
    roa: roa.ROA,
    roe: roe.ROE,
    roi: roi.ROI,
    debtToEq: Number(debtToEq["Debt/Eq"]),
    ltDebtToEq: Number(ltDebtToEq["LT Debt/Eq"]),
    institOwn: institOwn["Inst Own"],
    yearLow: yearLow["52W Low"],
    yearHigh: yearHigh["52W High"],
    dividend: Number(dividend.Dividend),
    dividendPercent: dividendPercent["Dividend %"],
    beta: Number(beta.Beta),
    targetPrice: Number(targetPrice["Target Price"]),
    analystRecommendation: Number(recommendation.Recom)
  }
}
